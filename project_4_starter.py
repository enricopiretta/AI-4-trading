...
get_ipython().system('{sys.executable} -m pip install numpy --upgrade')
...
# upgrade versions
get_ipython().system('sudo pip3 install numpy==1.18')
#!{sys.executable} -m pip3 install pandas --upgrade
get_ipython().system('sudo pip3 install pandas==0.19.2')
#!{sys.executable} -m pip install pandas --upgrade
print('Can you please help me figure out why this error pops up?')
print('Regards')
...
def fit_pca(returns, num_factor_exposures, svd_solver):
    ...
    pca = PCA(num_factor_exposures, svd_solver)
...
def factor_cov_matrix(factor_returns, ann_factor):
    ...
    #TODO: Implement function
    factor_cov_matrix = np.diag(ann_factor*factor_returns.var(axis=0, ddof=1))
...
def idiosyncratic_var_matrix(returns, factor_returns, factor_betas, ann_factor):
    ...
    #TODO: Implement function
    market_returns = pd.DataFrame(np.dot(factor_returns, factor_betas.T), returns.index, returns.columns)
    residual = returns - market_returns
    idiosyncratic_var_matrix = pd.DataFrame(ann_factor*np.diag(np.var(residual)), returns.columns, returns.columns)
...
def idiosyncratic_var_vector(returns, idiosyncratic_var_matrix):
    ...
    #TODO: Implement function
    idiosyncratic_var_vector = pd.DataFrame(np.diag(idiosyncratic_var_matrix.values), returns.columns)
